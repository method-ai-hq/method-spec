...
import {executionTools} from '../src/tool-connections.js';
...
test('Codex receives text and images through the normal bridge; private call data stays out of journal',async t=>{
...
(async()=>{
const args=process.argv.slice(2);const url=JSON.parse(args.find(x=>x.startsWith('mcp_servers.method_step.url=')).split('=').slice(1).join('='));
const headers={'content-type':'application/json',authorization:'Bearer '+process.env.METHOD_CODEX_TOOL_TOKEN};
...
(await import('node:fs')).writeFileSync(args[args.indexOf('--output-last-message')+1],JSON.stringify({value:r.result.content[0].text}));
})().catch(e=>{console.error(e);process.exitCode=1});
`,{mode:0o700});
 const config={allow_local_processes:true,models:{default:{backend:'codex',command:fake}},environment:{browser:'selected'},tools:{browser_navigate:definition}};
 let count=0;
