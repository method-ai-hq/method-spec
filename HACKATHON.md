...
| Method 3 execution and check design | New proposal | Designed; not implemented. |
| New validator and reference runner | Planned new work | Not implemented. |
...
"Before the hackathon, Method represented reusable procedures with agent instructions, typed inputs and outputs, state, and checks. Actions and language-based checks used separate agent runs. For harder game tasks, we are adding direct model calls and script execution so the procedure can choose where to spend reasoning time."
