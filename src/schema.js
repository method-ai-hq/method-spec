...
export const methodSchema = {
  ...
  ...object({
    format: { enum: ['method/3', 'method/3.1'] }, name: text, goal: text, run_prompt: text,
  ...
  $defs: {
    ...
    step: {
      ...object({
        name: text, purpose: text, reading: object({ inputs: text, outputs: text, output_name: text, condition: text, check: text, check_name: text }, []), in: map(ref), out: map({ $ref: '#/$defs/data' }),
