...
export const methodSchema = {
  ...
  ...object({
    format: { const: 'method/3' }, name: text, goal: text,
  ...
  $defs: {
    ...
    data: object(shapeProperties, ['type', 'description']),
    input: object({ ...shapeProperties, default: {} }, ['type', 'description']),
    execution: { oneOf: [run, call, agent] },
    check: { oneOf: [...exact, run, agent] },
    step: {
      ...object({
        name: text, purpose: text, in: map(ref), out: map({ $ref: '#/$defs/data' }),
        ...
        limits: object({ timeout_ms: positive, max_agent_turns: positive, max_model_requests: positive }, ['timeout_ms']),
      }, ['purpose', 'limits']),
...
export const configSchema = {
  ...
  ...object({
    ...
    models: map(object({ backend: { const: 'openai-responses' }, model: text, api_key_env: { type: 'string', pattern: '^[A-Z_][A-Z0-9_]*$' }, max_output_tokens: positive, reasoning_effort: { enum: ['none', 'minimal', 'low', 'medium', 'high', 'xhigh'] } }, ['backend', 'model', 'api_key_env', 'max_output_tokens'])),
